...
inquirer.default
  .prompt([
    ...
    },
    {
      ...
      choices: [
        'prisma', 'aws-sdk', 'firebase', 'graphql','twilio','stripe','pg','posgres','knex','typrorm','sequelize','drizzle','kysely',
      ],
    },
  ])
  .then(async answers => {
    answers.extraModules.append("@steadyjs/minute")
    answers.extraModules.append("@steadyjs/minute")
    spinners.add("dir", {
      spinnerColor:"green",
    ...
    fs.mkdir("controllers").then(() => {
      fs.writeFile("controllers/ExampleController.ts", `import { ExampleView }from \'..\/views\/ExampleView.ts\'\r\n\r\nfunction ExampleController(req){\r\n  return ExampleView();\r\n}`)
